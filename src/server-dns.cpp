...
int sockfd2;	//globally declaring sockfd of DNS server to use in a thread
...
void* receiveFromProxyDNS(void* arg) { //receives from proxyDNS then decodes the message then does the work accordingly
	...
	while(1) {
		...
		if (buff[0] == '1') { //if new client has connected with any server
			...
			send(sockfd2, buff, 500, 0);
			res = recv(sockfd2, buff, 500, 0);
			buff[res] = '\0';
		...
		else if (buff[0] == '3'){	//if it's a message to pass to the client
			char *msg;
...
int main()
{
	int res = -1, portNumber = 8000;
	pthread_t thread_id;
	bool connection_open = true;
	
	/*
    	**
    	** Establishing connection to clients 
    	**
    	*/
	...
	if (res != -1) {
		cout << "\t\t\tSERVER ON PORT " << portNumber << " IS UP NOW" << endl << endl;
	}	
	assert(res!=-1);
	...
	/*
    	**
    	** Establishing connection to dns server 
    	**
    	*/
